"""
Imagina que esta API es una lista de alertas de servidores:
La función load_movies() es como un bibliotecario que carga el alertamiento de servidores.
La función get_movies() muestra todo el alertamiento de servidores cuando alguien lo pide.
La función get_movie(id) es como si alguien preguntara por un libro específico por su código de identificación.
La función chatbot (query) es un asistente que busca Alertas según palabras clave y sinónimo.
La función get_movies_by_category (cagory) ayuda a encontrar alertas según su género (acción, comedia, etc.).
"""
...
def load_movies():
    ...
    df = pd.read_csv("Dataset/Alerts_report.csv")[['Customer', 'Customer Code', ' Node', 'IP Address', 'Component', 'Serial', 'AlertKey', 'Description', 'First Occurrence Date', 'Last Occurrence Date', 'Solution Date', 'Mainatenance Window',  'Manager', 'Agent', 'AlertGroup', 'Original Severity', ' WMFLAG', 'Server name', 'ADO', 'TicketGroup', 'TicketNumber', 'AmTicket', 'Operative System', 'Identifier, Serial']]
    
    # Renombramos las columnas para que sean más faciles de entender
    df.columns = ['Customer', 'Customer Code', ' Node', 'IP Address', 'Component', 'Serial', 'AlertKey', 'Description', 'First Occurrence Date', 'Last Occurrence Date', 'Solution Date', 'Mainatenance Window',  'Manager', 'Agent', 'AlertGroup', 'Original Severity', ' WMFLAG', 'Server name', 'ADO', 'TicketGroup', 'TicketNumber', 'AmTicket', 'Operative System', 'Identifier, Serial']]
    ']
...
app = FastAPI(title="Mi aplicación de Alertamiento de servidores", version="1.0.0")
...
# Cuando entremos en el navegador a http://127.0.0.1:8000/ veremos un mensaje de bienvenida
    return HTMLResponse('<h1>Bienvenido a la API de Alertamiento </h1>')
